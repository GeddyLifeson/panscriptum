...
## 0. OPEN THE SHIFT THE USUAL WAY

`escalation.py --status` → `state/MAINTENANCE_RUN.json` → `workorders.py --sweep`. Nothing was
halted by run #33 and nothing was lifted.
...
## 1. TWO THINGS FOR A PERSON, NOT FOR A RUN
...
## 2. THE FIRST THING TO ACTUALLY FIX: a HARD RULE 0 violation
...
## 3. THE QUEUE, BY RUNG (118 open at close of run #33)
...
work a run closes; both are waiting on §1.
...
## 4. ON THE 114 SWEEP FINDINGS

`handoff/sweep33/` holds all 17 reports and is the full record; every finding is filed, none
dropped. **Four BLOCKING findings were verified against source before filing and all four were
real** — but the remaining 110 are filed **as reported, not as confirmed**. Verify before acting.
...
## 5. IF THE QUEUE COMES BACK EMPTY

Run the sweep. Run #33's is `run33`, 108/108 modules, and `sweep_plan.missing('run33')` is empty
