...
5. **Live-verify `local_agent.py` tool-calling in a quiet GPU window** — the harness and
   gates are unit-verified; the end-to-end Ollama tool loop hit the batch's 503 wall tonight
   (now retried 4x/patient). Run: `python src/local_agent.py --task "Read src/lognames.py
   and list every constant."` and confirm tool_calls flow. If qwen3 never emits one, pull a
   tool-capable model from the list the error names.
6. **1,060 reopened Marvel entrypass batches**: confirm the pipeline is re-judging them
   (phase 2 progress in `state/pipeline_auto.log`).

## 2. Proposals (act only after a review look)

7. **Delete `identity.adjudicate()` (src/identity.py:321-367)** — dead code, superseded by
...
8. **Sweep-level by_axis result caching** — `sweep.py` re-derives `candidates()` over the
...
9. **`hostcheck._NULL_CACHE` to disk** — minor (1-2 HTTP calls per foreman round).
10. **manifest_builder/address per-entry normalize memo** — verified real but ~milliseconds;
   only worth doing if touching those files anyway.

## 3. Human decisions needed (owner)

11. **dandwiki.com (BUGS M1)**: build an HTML-path reader with a browser UA for its 4
...
12. **Disk (BUGS M2)**: ~5 GB free on the drive, floor is 10; the project is only ~1 GB of
    it. Clear non-project space or say where the corpus should move.
13. **Permanently hostless roll entries** (Clockwork Angels, Twilight Imperium, HAWX, …):
    rule whether they stay on the roll as owner-supplied-text candidates (ingest_doc now
    exists for exactly that) or come off.
14. **Paid burst lane**: 500-call cap stands, counter in `state/PAID_BURST.json`, spend line
