# Ròdais and the Rodos Atlas

The owner's constructed language for the island of Rodos and the island's map and history. There are
two things here:

1. **The language: `RODAIS.html`.** Open it in any browser. **Book** has the sounds, the grammar in
   ...
   the 18,500 most frequent in English is in it, unless it is a name, an abbreviation or not English. Ròdais is a Gaelic dialect with
...
2. **The map, timeline and history: `Rodos_Atlas/`.** Double-click `Rodos Atlas.bat` (or run
   `python atlas.py`). It opens in your browser with three tabs: **Annals** (all 1,515 dated events,
   filter by age, search), **Map** (Azgaar's Fantasy Map Generator itself, running the Rodos map) and
   **Book** (the legendarium: five prose ages, appendices, a gazetteer of all 505 towns). An event's
...
| File | What it is |
|---|---|
| `RODAIS.html` | The language book (built by `build_language.py` from the files below and `language_template.html`; needs `pip install markdown`). |
| `PHONOLOGY.md` | The sound system in full: consonant and vowel inventories with IPA (broad/slender, fortis/lenis, pre-aspiration, tense/lax sonorants), allophony, syllable structure, the helping vowel, stress and intonation, the sounds of the mutations, every spelling-to-sound rule, a key for English speakers. Where dialects differ Ròdais takes the older sound. |
| `GRAMMAR.md` | The grammar in brief: spelling, mutations, articles, nouns, adjectives, pronouns, verbs, the copula, word order, numbers, time, and how Ròdais names new things (§13). |
...
| `TEXTS.json`, `TEXTS.md` | Four everyday conversations and six tales with line-by-line English. |
...
| `LEXICON.md`, `LEXICON_EN.md` | The dictionary as text, Ròdais–English and English–Ròdais. |
...
| File | What it is |
|---|---|
| `Rodos_Atlas/` | The program (see above). Built by `build_atlas.py`. |
| `legendarium/` | The history: the dated annals, the five prose books, the appendices, the gazetteer, the reckoning of years from album release dates (`reckoning.py`, `albums.json`, `albums_src/`). `LEGENDARIUM.md` is the whole record as plain text. See `legendarium/README.md`. |
| `Rodos_finished.map` | The finished map. Open it in Azgaar's Fantasy Map Generator 1.153.1. |
| `Rodos_renamed.map`, `finish_map.py`, `MAP_CHANGES.md` | The map as the first renaming pass left it, the script that finishes the renaming through the engine, and every name it changed (425). |
| `build_atlas.py`, `atlas.py`, `atlas_template.html`, `atlas_style.css` | Rebuild `Rodos_Atlas/` from an Azgaar build, the map and the legendarium (instructions at the top of `build_atlas.py`). |
...
python build_language.py     # -> RODAIS.html
...
- **New things: loans from about 1800, kennings for coinages.** Things known before about 1800
  ...
  coined a word, Ròdais has its own kenning from old roots (GRAMMAR.md §13). A Scottish Gaelic
