...
| `reconcile/` | Every layer of the generated map read against the history: one proposal per layer (`religions`, `economy`, `state`, `military`, `land`, `markers_routes`, `heraldry` `.json`), each with what agreed, what conflicted, what was missing, and its `map_edits`, annals, gazetteer and text edits; `integration.json`, the integrator's own map edits and the proposal edits it sets aside; then `faiths.json` (the three faiths), `climate.json` (its mild temperatures, no glacier and no ice), `dubhan.json` (dubhan the traded fuel) `loose_ends.json` (the two routes cut into road and trail pieces) and `tales.json` (the tellings of the Old Faith read against the Gaelic tales: Macha's wood, the wood of the two horses), `calendar.json` (the map's calendar set to the Dubhan Era, year 27, and the war's years and the regiments' and battlefields' dated notes in that era), and last `place.json` (the scale, 0.14 miles to the unit, an island about the size of Northern Ireland, and the frame, alone in the Atlantic west of the Hebrides, see `../eras/RESCALE_LOG.md`; and the population scale, 38 people to the unit in the country and every town's people set outright, some 1.8 million on the island, a third in towns and the capital the largest, with the regiments cut to match, see `../eras/POP_LOG.md`); `INTEGRATION.md`, how the proposals were combined and every conflict decided. |
...
| `town_previews.json` | Every burg's town-plan link (Watabou's City or Village Generator), as the Atlas's own Azgaar builds it from the map (rebuilt at the map's population scale by `../eras/pop_sweep.py`). The forts get a citadel town with no walls or square. |
