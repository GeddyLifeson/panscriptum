import{j as e}from"./utils-Cob8vHf9.js";import{d as t,p as n,t as r}from"./layers-Bh4OWBXX.js";import{i}from"./tooltips-BTSHGd98.js";import{t as a}from"./controllers-DnVd8NhQ.js";import{ct as o,kt as s,lt as c,mt as l,st as u}from"./index-DsIn6sTp.js";function d(){customization||(u(`.stable`),f(),p(),$(`#labelGroupsConfigurator`).dialog({title:`Configure Label Groups`,resizable:!1,maxHeight:Math.max(window.innerHeight-40,300),position:{my:`right top`,at:`right-10 top+10`,of:`svg`,collision:`fit`},close:E,buttons:{Apply:()=>{e(`labelGroupsForm`).requestSubmit()},Add:()=>{e(`labelGroupsBody`).insertAdjacentHTML(`beforeend`,g({name:``,type:`state`,zoom:{min:null,max:null}},!0,0))},Restore:()=>{let t=Labels.getDefaultOptions();e(`labelsResizeOnZoom`).checked=t.resizeOnZoom,e(`labelsShowAll`).checked=t.showAll,p(t.groups)},Cancel:function(){$(this).dialog(`close`)}}}))}function f(){c(`labelGroupsConfigurator`);let t=`<div id="labelGroupsConfigurator" class="dialog stable">
    <form id="labelGroupsForm">
      ...
      <div style="display:flex; gap:1.2em; align-items:center; margin:.6em 0 0">
        ...
        <button type="button" name="missing" data-group="${e}" class="icon-list-bullet"
          data-tip="Show labels of the ${e} group in Labels Overview to reassign them"></button>`).join(`, `)}function h(){let e=new Map,n=t=>e.set(t,(e.get(t)??0)+1);return t().forEach(e=>void n(e.group)),e}function g(e,t=!1,n=0){let r=[`auto`,`short`,`full`],i=!!e.isDefault,a=i?`Default group for this type, can't be renamed`:`Group name. Must start with a letter or underscore, followed by letters, digits, underscores, or dashes`,o=x(e.type),c=o?`Name display mode: auto picks the best fit, short/full force a specific name form`:`Name display mode is only applicable to States and Provinces`,u=[...s.keys()].sort();return`<tr data-group="${t?``:e.name}" data-is-default="${i?`1`:``}">
      ...
      <td data-tip="${c}"><select name="mode" ${o?``:`disabled`}>
    ...
    </tr>`}function _(e){let t=e.target;if(!(t instanceof HTMLSelectElement)||t.name!==`type`)return;let n=t.closest(`tr`)?.querySelector(`[name="mode"]`);if(!n)return;let r=x(t.value);n.disabled=!r,r||(n.value=`auto`)}function v(e){let t=e.target.closest(`button[name='missing']`);t?.dataset.group&&a.LabelsOverview.open(t.dataset.group)}function y(e){let t=e.target.closest(`button[name]`);if(!t||t.disabled)return;let n=t.closest(`tr`);if(n){if(t.name===`up`){let e=n.previousElementSibling;e&&n.parentNode.insertBefore(n,e);return}if(t.name===`down`){let e=n.nextElementSibling;e&&n.parentNode.insertBefore(e,n);return}if(t.name===`style`){let e=n.querySelector(`[name="name"]`).value.trim();e&&editStyle(`labels`,e);return}if(t.name===`list`){let e=n.dataset.group;e&&a.LabelsOverview.open(e);return}t.name===`remove`&&b(n)}}function b(t){if(e(`labelGroupsBody`).children.length<2){i(`At least one group should be defined`,!1,`error`);return}o({title:`Remove Label Group`,message:`Remove the group? This won't affect labels unless the changes are applied.`,confirm:`Remove`,onConfirm:()=>{t.remove(),S()}})}function x(e){return[`state`,`province`].includes(e)}function S(){let t=e(`labelGroupsForm`),n=Array.from(t.querySelectorAll(`input[name="name"]`)),r=n.map(e=>e.value.trim());n.forEach(e=>{if(e.disabled){e.setCustomValidity(``);return}let t=``,n=e.value.trim(),i=/^[\p{L}_][\p{L}\p{N}_-]*$/u.test(n),a=r.filter(e=>e===n).length===1;i||(t=`Group name must start with a letter or underscore and not contain special characters`),a||(t=`Group name should be unique`),e.setCustomValidity(t)});let i=t.checkValidity();return i||t.reportValidity(),i}function C(t){if(t.preventDefault(),!S())return;let a=Array.from(e(`labelGroupsBody`).children);if(!a.length)return void i(`At least one group should be defined`,!1,`error`);let o=new Set;a.forEach(e=>{let t=e.dataset.group,r=w(e);o.add(r.name),r.name!==t&&(t?(T(t,r.name),styles.labels.groups[r.name]=styles.labels.groups[t],delete styles.labels.groups[t]):styles.labels.groups[r.name]=n(r))}),options.map.labels.groups.forEach(e=>{if(o.has(e.name))return;let t=Labels.getFallbackGroup(e.type);T(e.name,t.name),delete styles.labels.groups[e.name]}),options.map.labels.groups=a.map(w),options.map.labels.resizeOnZoom=e(`labelsResizeOnZoom`).checked,Options.set(t=>t.app.labels.showAll=e(`labelsShowAll`).checked);for(let e of options.map.labels.groups)styles.labels.groups[e.name]??=n(e);r.draw(`labels`),$(`#labelGroupsConfigurator`).dialog(`close`)}function w(e){let t=e.querySelector(`[name="name"]`).value.trim(),n=e.querySelector(`[name="type"]`).value,i=e.querySelector(`[name="active"]`).checked,a=e.querySelector(`[name="mode"]`).value,o=e.querySelector(`[name="zoom-min"]`),s=e.querySelector(`[name="zoom-max"]`),c=e.querySelector(`[name="dependency"]`).value.trim(),l={name:t,type:n,zoom:{min:o.value===``?null:o.valueAsNumber,max:s.value===``?null:s.valueAsNumber}};return i||(l.active=!1),a!==`auto`&&(l.mode=a),r.has(c)&&(l.layerDependency=c),e.dataset.isDefault===`1`&&(l.isDefault=!0),l}function T(e,n){let r=t();for(let{type:t,entityId:i,group:a}of r)a===e&&Labels.setGroup({type:t,entityId:i,group:n})}function E(){c(`labelGroupsConfigurator`)}var D={open:d};export{D as LabelGroupsConfigurator};