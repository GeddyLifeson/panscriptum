import{j as e}from"./utils-Cob8vHf9.js";import{t}from"./layers-Bh4OWBXX.js";import{i as n}from"./tooltips-BTSHGd98.js";import{t as r}from"./controllers-DnVd8NhQ.js";import{ct as i,lt as a,st as o,ut as s}from"./index-DsIn6sTp.js";var c=/^[\p{L}_][\p{L}\p{N}_-]*$/u;function l(){customization||(o(`.stable`),u(),f(),$(`#burgGroupsEditor`).dialog({title:`Configure Burg groups`,resizable:!1,position:{my:`center`,at:`center`,of:`svg`},close:d,buttons:{Apply:()=>{e(`burgGroupsForm`).requestSubmit()},Add:()=>{let t={name:``,order:Math.max(0,...options.map.burgs.groups.map(({order:e})=>e))+1,active:!0};e(`burgGroupsBody`).insertAdjacentHTML(`beforeend`,p(t))},Restore:()=>{f(Burgs.getDefaultGroups())},Cancel:function(){$(this).dialog(`close`)}}}))}function u(){a(`burgGroupsEditor`),e(`dialogs`).insertAdjacentHTML(`beforeend`,`<div id="burgGroupsEditor" class="dialog stable">
    ...
    <div style="padding: 0.5em 0; font-style: italic;">
    ...
    </tr>`}function m(e,t){let r=e.previousElementSibling.value,i=r?r.split(`,`).map(e=>+e):[],a=t.filter(e=>e.i&&!e.removed).map(({i:e,name:t,fullName:n,color:r})=>`
        <tr data-tip="${t}">
          <td>
      ...
      </table>`,$(`#alert`).dialog({width:`fit-content`,title:`Limit group`,buttons:{Invert:()=>{alertMessage.querySelectorAll(`input`).forEach(e=>{e.checked=!e.checked})},Apply:function(){let t=Array.from(alertMessage.querySelectorAll(`input`)),r=t.reduce((e,t)=>(t.checked&&e.push(t.dataset.i),e),[]);if(!r.length)return n(`Select at least one element`,!1,`error`);let i=r.length===t.length;e.previousElementSibling.value=i?``:r.join(`,`),e.innerHTML=i?`all`:`some`,$(this).dialog(`close`)},Cancel:function(){$(this).dialog(`close`)}}})}function h(t){let n=t.previousElementSibling.value,r=n?JSON.parse(n):{},i=[{name:`capital`,icon:`icon-star`},{name:`port`,icon:`icon-anchor`},{name:`palace`,icon:`icon-chess-rook`},{name:`walls`,icon:`icon-fort-awesome`},{name:`plaza`,icon:`icon-store`},{name:`temple`,icon:`icon-chess-bishop`},{name:`shanty`,icon:`icon-campground`}],a=i.map(({name:e,icon:t})=>`
