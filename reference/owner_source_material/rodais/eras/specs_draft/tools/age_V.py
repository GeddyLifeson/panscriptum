...
def build():
    ...
    d['cultures'] = [
        ...
        collections.OrderedDict([('key', 'humans'), ('name', None), ('name_en', 'the humans of an Tìr Thall (called the Tuathaich from the Severance)'), ('master_culture', 1),
                                 ('provinces', [105]),
                                 ('note', 'A few thousand left on the island: the concession families of the north-west, and the families of Ros bheag, Cuan shean, Baile dhìreach and the south-west now in the columns going north. Their homeland is called an Tìr Thall; their own name for it was never written down.'),
    ...
    # ---------------------------------------------------------------- burgs
    SPECIAL = {
        19: dict(population=514, kind='town', role='the custodians\' hall, its locks broken; the bridge barricaded with coal-carts in the rising; hungry since the winter, the grain shared by household', cites=['IV-0331', 'IV-0354', 'IV-0376']),
        27: dict(population=1300, kind='city', role='the capital\'s great harbour: the Residency above it stripped, the Company\'s quays and deep-water quay, the burned counting-house and the Mission chapel; the harbour quarter opened its gates to the council', cites=['IV-0031', 'IV-0044', 'IV-0211', 'IV-0356', 'IV-0360', 'IV-0362']),
        354: dict(population=800, kind='city', role='the heart of the Company\'s pits: overseers\' town, weigh-house burned, the store taken at first light by Mòrag nic Iain\'s crews; the coal line runs from here', cites=['IV-0040', 'IV-0092', 'IV-0243a', 'IV-0325', 'IV-0358']),
        44: dict(population=400, kind='town', role='the strike town and home of the Lamp-Watch: the walled pithead held eleven days, the book of complaints', cites=['IV-0166', 'IV-0208', 'IV-0213', 'IV-0234', 'IV-0327']),
        419: dict(population=89, kind='town', role='the silver guild\'s town under the Silver Compact; rose for the council and escorted the Company\'s agents to the coast', cites=['IV-0103', 'IV-0171', 'IV-0337']),
        135: dict(population=400, kind='town', role='the camp of the deep galleries under the hill, houses built of spoil; the green acre where the Green Death was first marked; the Sloc Mòr is here', cites=['IV-0144', 'IV-0153']),
        224: dict(population=83, kind='large village', role='the first town drawn on paper, a Company camp on a grid; two streets pulled down for a spoil-tip', cites=['IV-0087', 'IV-0318']),
        383: dict(population=60, kind='large village', role='the Company\'s camp of timber huts at the ford below the new workings, the first settlement made for the pit alone', cites=['IV-0077', 'IV-0163']),
        476: dict(population=270, kind='town', walls=True, role=HUMAN_TOWNS[476], cites=['IV-0116', 'IV-0199', 'IV-0287', 'IV-0346']),
        54: dict(population=45, kind='large village', role=HUMAN_TOWNS[54], cites=['IV-0347']),
        160: dict(population=45, kind='large village', walls=True, role=HUMAN_TOWNS[160], cites=['gazetteer B160', 'IV-0288']),
        63: dict(population=400, kind='town', role='the last seat of the Administration, hungry behind its walls; the last ship from the east anchored here; its human families gone north in the columns', cites=['IV-0363', 'IV-0365', 'IV-0368', 'IV-0371']),
        282: dict(population=600, kind='town', role='where the humans first came ashore and whence the last Commissioner sailed; its custodians shut the harbour to the Company\'s ships', cites=['IV-0002', 'IV-0366', 'IV-0374']),
        304: dict(population=270, kind='town', role='the truce was agreed here, on the edge of the concession', cites=['IV-0369']),
        303: dict(population=170, kind='town', role='the greatest town of the north-west, named in the truce to receive the columns; its Dia-thìrich given the choice of staying or going south', cites=['IV-0370']),
        313: dict(population=130, kind='large village', role='its coal stage shelled by a Company steamship from the river and the fishing town behind it set burning; three dead', cites=['IV-0090', 'IV-0342']),
        255: dict(population=400, kind='town', role='the rope-walks turned to winding-rope; the constables\' barracks; it shut its gates behind Ashdown and nursed his wounded', cites=['IV-0082', 'IV-0283', 'IV-0336']),
        105: dict(population=55, kind='large village', role=MIXED_TOWNS[105], cites=['IV-0022', 'IV-0110', 'IV-0367']),
        142: dict(population=330, kind='town', role=MIXED_TOWNS[142], cites=['IV-0117']),
        143: dict(population=330, kind='town', role=MIXED_TOWNS[143], cites=['IV-0118', 'IV-0122a', 'IV-0262a']),
        305: dict(population=200, kind='town', role='the hunger was worst here, within sight of the new beacon; the Mission kitchen; the human labourers\' vigil of the Grey Night in the fields near it', cites=['IV-0195', 'IV-0199', 'IV-0262']),
        246: dict(population=170, kind='large village', role='the cellars of the custodians\' rolls, beyond the Residency\'s reach; the rolls of the age sent here at the war\'s end; the Line of Aisling found', cites=['IV-0248', 'IV-0251a', 'IV-0376']),
        152: dict(population=170, kind='large village', role='the Company\'s second coal port, end of the mule tram-road', cites=['IV-0148', 'IV-0236']),
        351: dict(population=200, kind='town', role='the Company\'s coal quay, seized by the crews and its mouth blocked with two sunk barges', cites=['IV-0041', 'IV-0326']),
        397: dict(population=400, kind='large village', role='the deep-water quay for the eastern coal; a village of forty houses become a town of four hundred', cites=['IV-0212']),
        26: dict(population=800, kind='city', role='the Company\'s first warehouse, broken open for the council\'s companies', cites=['IV-0033', 'IV-0332']),
        77: dict(population=800, kind='city', role='the druid school\'s hall shut; the grey fever took the Administration\'s physician; the masters\' houses opened as sick-rooms', cites=['IV-0186', 'IV-0247', 'IV-0278']),
        132: dict(population=600, kind='town', role='Ìomhar Seabhag\'s hawk-shrine, where the hawk company was called', cites=['IV-0284', 'IV-0348']),
        489: dict(population=2000, kind='city', role='the hawk company mustered here and held the road from Ros bheag to the north-west', cites=['IV-0349']),
    }
    ABSENT = {
    ...
    s.new_burg('V-N03', None, 'the camp of Cnoc fhiadhaich workings', ('burg', 280),
               population=55, kind='mining camp', role='the Company\'s working opened round Doire mhòr\'s fields; the gallery that killed eleven; the coughing sickness',
