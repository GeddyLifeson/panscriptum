...
SYSTEM = """You read one sentence from a fiction wiki and report a CONTEST OUTCOME.

Given the sentence and the entity it is filed under, name who WON and who LOST. Both must be
named or clearly referred to in the sentence itself.
...
SCHEMA = {
    ...
    "properties": {
        "outcomes": {"type": "array", "items": {
            "type": "object",
            "properties": {"winner": {"type": "string"}, "loser": {"type": "string"}},
...
def extract(rows, batch=8, limit=None, workers=8):
    ...
    def work(chunk):
        ...
        local = []
        for i, o in enumerate((got or {}).get("outcomes", [])):
            src = chunk[min(i, len(chunk) - 1)]
